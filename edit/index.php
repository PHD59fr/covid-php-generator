...
if ($_POST["password"] && $_POST["firstname"] && $_POST["lastname"] && $_POST["birthday"] && $_POST["birthplace"] && $_POST["address"] && $_POST["city"] && $_POST["zipcode"]) {
    header('Content-Type: application/json; charset=utf-8');
    if ($htpasswd->userExists($username)) {
	$ts = time();
	...
	if ($retRead) {
	    $htpasswd->updateUser($username, $password,Htpasswd::ENCTYPE_SHA1);
	    echo '{"message": "L\'utilisateur a été modifié avec succès", "type": "success"}';
	}else{
	    echo '{"message": "Une érreur est survenue : '.readUserdata($username,$password).'", "type": "error"}';
	}
    }else{
	    echo '{"message": "Une érreur est survenue", "type": "error"}';
    }
    exit();
}
...
<html>
	...
	<div class="containeri-fluid rounded col-md-3 mx-auto text-center" style="border: 2px solid silver;margin-top:10px;">
	    <h5>Formulaire de modification d'utilisateur</h5>
	    <form class="needs-validation" id="editing" action="/edit/" method="post" novalidate>
		<div>
		    <label for="password">Mot de passe</label>
		    <input type="password" class="form-control" id="password" name="password" placeholder="123456" value="<?php echo $password; ?>" required>
		    <div class="invalid-feedback">
			Merci d'indiquer votre mot de passe
		    </div>
		    <label for="firstname">Nom</label>
		    <input type="text" class="form-control" id="firstname" name="firstname" placeholder="Doe" value="<?php echo $firstname; ?>" required>
		    <div class="invalid-feedback">
			Merci de spécifier un nom valide
		    </div>
		    <label for="lastname">Prénom</label>
		    <input type="text" class="form-control" id="lastname" name="lastname" placeholder="John" value="<?php echo $lastname; ?>" required>
		    <div class="invalid-feedback">
			Merci de spécifier un prénom valide
		    </div>
		    <label for="birthday">Date de naissance</label>
		    <input type="date" class="form-control" id="birthday" name="birthday" placeholder="01/01/1960" value="<?php echo $birthday; ?>" required>
		    <div class="invalid-feedback">
			Merci de spécifier une date de naissance valide
		    </div>
		    <label for="birthplace">Lieu de naissance</label>
		    <input type="text" class="form-control" id="birthplace" name="birthplace" placeholder="Paris" value="<?php echo $birthplace; ?>" required>
		    <div class="invalid-feedback">
			Merci de spécifier un lieu de naissance valide
		    </div>
		    <label for="address">Adresse</label>
		    <input type="text" class="form-control" id="address" name="address" placeholder="55 Rue du Faubourg Saint-Honoré" value="<?php echo $address; ?>" required>
		    <div class="invalid-feedback">
			Merci de spécifier une adresse valide
		    </div>
		    <label for="city">Ville</label>
		    <input type="text" class="form-control" id="city" name="city" placeholder="Paris" value="<?php echo $city; ?>" required>
		    <div class="invalid-feedback">
			Merci de spécifier une ville valide
		    </div>
		    <label for="zipcode">Code postal</label>
		    <input type="number" class="form-control" id="zipcode" name="zipcode" placeholder="75008" value="<?php echo $zipcode; ?>" required>
		    <div class="invalid-feedback">
...
(function() {
  ...
  $("#preference").val('<?php echo $preference; ?>');
})();
</script>
    </body>
</html>
